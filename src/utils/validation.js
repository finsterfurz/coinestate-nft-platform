...
// KYC Data Validation
export const kycDataSchema = Joi.object({
...
export const nftMetadataSchema = Joi.object({
...
export const validateAndSanitize = (input, schema) => {
  try {
    ...
    return { isValid: false, value: null, error: err.message };
  }
};
 ...
 */
export const generateSecureToken = (length = SECURITY_CONSTANTS.CSRF_TOKEN_LENGTH) => {
...
export const validateTransaction = (tx) => {
  ...
  const { value, gasLimit, gasPrice } = result.value;
